...
- **No target leakage in producer aggregates.** Producer mean-rating / std / n_reviews features are computed **on the training fold only** and **leave-one-wine-out** — a wine's own ratings never enter its producer features. Train-fold-only is not enough: self-inclusion within train made `producer_mean_rating` a near-copy of the label at small wineries, and the early-stopping fold (carved from train) rewarded exploiting it — the Phase 5 audit found the model losing to the plain winery baseline on held-out wines because of this. Any group-statistic feature (region means, grape means, …) must follow the same rule.
